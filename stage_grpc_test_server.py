...
# -*- coding: utf-8 -*-
"""
三轴台 StageService gRPC 测试服务器（Python）
============================================
与 proto/stage.proto 一致，供 **三轴台测试装置** 面板 / StageReceiverBackend 联调。

**命名区分（勿混淆）：**
  - `grpc_test_server.py`  → **被测设备数据** DeviceDataService（Subscribe/SendCommand）
  - `stage_grpc_test_server.py` → **三轴台工装** StageService（Connect / PositionStream / Jog / …）

默认监听 **0.0.0.0:50052**，避免与本仓库 `grpc_test_server.py` 默认端口 50051 同时启动时冲突。
控制端请将 gRPC 地址填为同一端口，例如 `127.0.0.1:50052`。

用法：
  python stage_grpc_test_server.py [--port 50052] [--host 0.0.0.0]

绑定失败（Failed to bind）常见原因：**端口已被占用**（例如上次未退出的本脚本、或其它程序）。
可换端口 `--port 50053`，或查占用：`netstat -ano | findstr :50052`（Windows）。

依赖：grpcio、protobuf（与 `grpc_test_server.py` 相同环境即可）
桩代码目录：proto/generated_py/stage_pb2*.py（修改 stage.proto 后需重新 protoc）
"""
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例：
  python stage_grpc_test_server.py                  # 默认端口 50052
  python stage_grpc_test_server.py --port 50051    # 独占端口时可用 50051
    ...
    parser.add_argument(
        "--host",
        default="0.0.0.0",
        help="监听地址（默认 0.0.0.0；若绑定异常可试 127.0.0.1）",
    )
